...
<nav>
	...
	if ($results->currentPage + $range >7) : ?>
			<li class="page-item"><a class="page-link" href="{{ url()->current() . '?page=' . '1' }}">First</a></li>
	<?php endif ?>
	<?php if ($results->currentPage > 1) : ?>
			<li class="page-item"><a class="page-link" href="{{ url()->current() . '?page=' . ($results->currentPage - 1) }}">Previous</a></li>
	<?php endif ?>
	<?php
		...
		<?php
				...
				{
					?>
 			<li class="page-item"><a class="page-link" href="{{ url()->current() . '?page=' . $x }}"><?= $x?></a></li>
		<?php
			}
		}
	} ?>
	...
	if ($results->currentPage<$results->numberOfPages) : ?>
			<li class="page-item"><a class="page-link" href="{{ url()->current() . '?page=' . ($results->currentPage+1) }}">Next</a></li>
	<?php endif ?>
	<?php
	if (($results->currentPage + $range)<$results->numberOfPages) : ?>
			<li class="page-item"><a class="page-link" href="{{ url()->current() . '?page=' . ($results->numberOfPages) }} ">Last</a></li>
	<?php endif ?>
	</ul>
